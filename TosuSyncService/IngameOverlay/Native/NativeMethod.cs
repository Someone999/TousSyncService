...
namespace TosuSyncService.IngameOverlay.Native;
...
static class NativeMethod
{
    ...
    [DllImport("kernel32.dll")]
    public static extern LPVOID VirtualAllocEx(HANDLE hProcess, HANDLE lpAddress, int dwSize, int flAllocationType,
        int flProtect);
    ...
    [DllImport("kernel32.dll")]
    public static extern HANDLE CreateRemoteThread(HANDLE hwnd, int attrib, int size, HANDLE address, HANDLE par,
    ...
    [DllImport("Kernel32.dll")]
    public static extern long WriteProcessMemory(HANDLE hProcess, LPVOID addrMem, LPVOID buffer, int intSize,
